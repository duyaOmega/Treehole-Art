...
版本号的唯一来源是 `package.json`；Vite 会把它自动写入用户脚本的 `@version`。发版前：

1. 在 `package.json` 中更新版本号。
2. 在 `CHANGELOG.md` 顶部添加对应的 `## [x.y.z] - YYYY-MM-DD` 条目；该条目会成为 GitHub Release 说明。
3. 提交修改并推送到当前分支。完整发布要求工作区干净，且本地 `HEAD` 已存在于 `origin` 的同名分支。
4. 先演练构建：
...
4. 向 `s-bj-2937-artdoge-1258813047/release/Treehole-Art.user.js` 上传产物。
...
`--cdn-only` 仍会重新测试、构建并校验产物，但不会检查 Git 状态，也不会创建 GitHub Release。
